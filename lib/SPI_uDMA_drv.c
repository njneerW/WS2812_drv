				...
				uint8_t *pui8DoneVar)
{
	...
	// since the data is coming from a uint8_t buffer.  The destination increment
	// is none since the data is to be written to the SSI data register.  The
	// arbitration size is set to 4, which doesn't matter much since we're only
	// running one uDMA transfer.  If we were running multiple transfers through
	// the uDMA engine, we would want to increase this enough to insure that the
	// full LED strip worth of data goes out before the uDMA engine services
	// another channel.
	// TOD: see last sentence.
	//
	ROM_uDMAChannelControlSet(UDMA_CHANNEL_SSI1TX | UDMA_PRI_SELECT,
